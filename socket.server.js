    ...
    io.on('connection', socket => {
        ...
        socket.on('messageRead', async (data) => {
            try {
                ...
                await Chat.updateMany(query, { $addToSet: { readBy: receiverId } });

                // Update unread counts for both sides
                await emitUpdatedUnreadCounts(io, senderId, receiverId, product);

                socket.emit('messageReadByReceiver', { success: true });
            } catch (err) {
