...
const Advertisement = require('../../models/advertisementModel'); // ← make sure to import
...
// controllers/admin/reportController.js  (or wherever getReports is)
...
exports.getReports = async (req, res) => {
    try {
        ...
        // Advertisement summary statistics
        const adMatch = { ...dateFilter };
        ...
        const adStats = await Advertisement.aggregate([
            { $match: adMatch },
            {
                $group: {
                    _id: null,
                    totalAdsCreated: { $sum: 1 },
                    totalAdsPaid: { $sum: { $cond: [{ $eq: ['$paymentStatus', 'paid'] }, 1, 0] } },
                    totalRevenueFromAds: {
                        $sum: { $cond: [{ $eq: ['$paymentStatus', 'paid'] }, '$totalAmount', 0] },
                    },
                    approvedAds: { $sum: { $cond: [{ $eq: ['$approvalStatus', 'approved'] }, 1, 0] } },
                    rejectedAds: { $sum: { $cond: [{ $eq: ['$approvalStatus', 'rejected'] }, 1, 0] } },
                    cancelledAds: { $sum: { $cond: [{ $eq: ['$status', 'cancelled'] }, 1, 0] } },
                    activeAds: { $sum: { $cond: [{ $eq: ['$status', 'active'] }, 1, 0] } },
                    completedAds: { $sum: { $cond: [{ $eq: ['$status', 'completed'] }, 1, 0] } },
                },
            },
        ]);

        const adReport = adStats[0] || {
            totalAdsCreated: 0,
            totalAdsPaid: 0,
        ...
        // Currently active advertisements (real-time)
        const currentlyActiveAds = await Advertisement.countDocuments({
            status: 'active',
            paymentStatus: 'paid',
        ...
        // Detailed advertisements list for table
        const advertisements = await Advertisement.find(adMatch)
            .populate('seller', 'name email')
            .populate('product', 'title')
            .sort({ createdAt: -1 })
            .limit(100); // adjust limit as needed

        // Calculate totals from detailed list (for filtered period)
        let totalAdRevenueFiltered = 0;
        let totalApprovedFiltered = 0;
        let totalRejectedFiltered = 0;

        advertisements.forEach(ad => {
            if (ad.paymentStatus === 'paid') {
                totalAdRevenueFiltered += ad.totalAmount;
            }
            if (ad.approvalStatus === 'approved') totalApprovedFiltered++;
            if (ad.approvalStatus === 'rejected') totalRejectedFiltered++;
        });
        ...
        const adTrend = await Advertisement.aggregate([
            {
                $match: {
                    paymentStatus: 'paid',
        ...
        res.render('reports', {
            ...
            adReport,
            currentlyActiveAds,
            advertisements,              // detailed list for table
            adTrend,
            totalAdRevenueFiltered,
            totalApprovedFiltered,
            totalRejectedFiltered,
        });
    } catch (error) {
        console.error('Reports error:', error);
        req.flash('red', error.message || 'Something went wrong while loading reports');
        res.redirect('/admin');
    }
};
