...
// exports.getFirstUserDiscountStatus = async (req, res) => {
//     try {
...
//         // 1. Check if user has ANY previous booking (successful ya pending bhi count kar sakte ho)
//         const hasPreviousBooking = await Booking.exists({
//             user: userId,
//             // Optional: status: { $in: ['pending', 'confirmed', 'completed'] }  ← agar sirf confirmed/completed count karna ho to uncomment
//         });

//         // 2. AdminCommission se discount value le lo
//         const commission = await AdminCommission.findOne().lean();

//         let discountPercent = 0;
//         if (commission && commission.firstUserDiscount) {
//             discountPercent = Number(commission.firstUserDiscount);
//         }
...
//                 message: "First booking discount already used (you have previous bookings)",
//             });
//         }
...
//             message:
//                 discountPercent > 0
//                     ? `Congratulations! Get ${discountPercent}% OFF on your first booking`
//                     : "No first-user discount configured right now",
//         });
//     } catch (err) {
//         console.error("Error in getFirstUserDiscountStatus:", err);
//         return res.status(500).json({
//             success: false,
//             message: "Server error while checking discount eligibility",
//             error: err.message,
//         });
//     }
// };
...
exports.getFirstUserDiscountStatus = async (req, res) => {
    try {
        ...
        // 1. Check if user has ANY previous booking
        const hasPreviousBooking = await Booking.exists({
            user: userId,
            // Optional: status: { $in: ['pending', 'confirmed', 'completed'] }
        });

        // 2. AdminCommission se discount value + commissionType lo
        const commission = await AdminCommission.findOne().lean();

        const discountValue  = commission?.firstUserDiscount ? Number(commission.firstUserDiscount) : 0;
        // commissionType hi decide karta hai discount % hai ya fixed $
        const discountType   = commission?.commissionType || 'percentage';
        ...
        // 4. Eligible response — message bhi type ke hisaab se
        const message = discountValue > 0
            ? discountType === 'percentage'
                ? `Congratulations! Get ${discountValue}% OFF on your first booking`
                : `Congratulations! Get $${discountValue} OFF on your first booking`
            : "No first-user discount configured right now";
        ...
        return res.status(200).json({
            ...
            discountType: discountType,   // 'percentage' or 'fixed' — from commissionType
            message: message,
        });
