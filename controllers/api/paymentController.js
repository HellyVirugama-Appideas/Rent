...
//         if (!latitude || !longitude) {
//             return next(createError.BadRequest('User location required for delivery cost.'));
//         }
...
//         let rentalDays = 1;
//         if (Array.isArray(booking.bookedDates) && booking.bookedDates.length > 0) {
//             rentalDays = booking.bookedDates.length;
//         } else if (booking.startDate && booking.endDate) {
//             try {
//                 const start = new Date(booking.startDate);
...
//         const depositAmount = product.deposit ? Number(product.depositAmount || 0) : 0;
...
//             firstUserDiscountAmount = (finalAmount * firstUserDiscountPercentage) / 100;
//             finalAmount -= firstUserDiscountAmount;

//             console.log(`First-time discount applied: ${firstUserDiscountPercentage}% ($${firstUserDiscountAmount.toFixed(2)})`);
//         }

//         // Commission only on rental price (not on deposit or delivery)
//         const commissionAmount = calculateCommission(rentalPrice, commissionSettings);

//         // Stripe fees
//         const stripeProcessingFee = calculateStripeProcessingFee(finalAmount);
//         const stripeTransferFee = calculateStripeTransferFee(rentalPrice);
//         const stripeTotalFee = stripeProcessingFee + stripeTransferFee;

//         // Owner payout (rental - commission)
//         const ownerPayoutAmount = rentalPrice - commissionAmount;
//         const netOwnerPayout = ownerPayoutAmount; // platform absorbs Stripe fees

//         // Final amount in cents for Stripe (must be integer)
//         const stripeAmountCents = Math.round(finalAmount * 100);

//         console.log('Final amount sent to Stripe (cents):', stripeAmountCents);
//         console.log('Final amount in AUD:', finalAmount.toFixed(2));
...
//                 firstUserDiscount: firstUserDiscountAmount.toString(),
//                 firstUserDiscountPercentage: firstUserDiscountPercentage.toString(),
//                 isFirstTimeUser: isFirstTimeUser.toString(),
//                 originalAmount: originalAmount.toString()
//             },
//             description: `Rental payment for ${product.title}${isFirstTimeUser ? ' (First User Discount)' : ''}`,
//             automatic_payment_methods: { enabled: true }
//         });

//         // Save payment record with correct discounted amount
//         const payment = await Payment.create({
//             booking: booking._id,
//             renter: booking.user._id,
//             owner: product.user,
//             product: product._id,
//             totalAmount: finalAmount,                      // discounted
...
//         // Response with correct values
//         res.status(200).json({
//             success: true,
//             clientSecret: paymentIntent.client_secret,
//             paymentIntentId: paymentIntent.id,
//             amount: finalAmount,                           // discounted – this is what Stripe will charge
//             originalAmount,
//             rentalAmount: rentalPrice,
//             depositAmount,
//             deliveryCharge,
...
//             },
//             message: isFirstTimeUser
//                 ? `First time discount: ${firstUserDiscountPercentage}% OFF! Original: $${originalAmount.toFixed(2)}, Pay: $${finalAmount.toFixed(2)} (Saved: $${firstUserDiscountAmount.toFixed(2)})`
//                 : `Total payment: $${finalAmount.toFixed(2)} (Rental: $${rentalPrice.toFixed(2)} + Deposit: $${depositAmount.toFixed(2)} + Delivery: $${deliveryCharge.toFixed(2)})`,
//             payment
//         });
//     } catch (error) {
//         console.error('Error creating payment intent:', error);
